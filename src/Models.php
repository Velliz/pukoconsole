...
class Models
{
    ...
    public function __construct($root, $act, $model_name, $schema)
    {
        ...
        switch ($act) {
            ...
            case 'remove':
                break;
            default:
                die($this->Prints('Command not valid!', true, 'light_red'));
        }
    }
    ...
    {
        if (empty($model_name)) {
            die($this->Prints('Model name required!', true, 'light_red'));
        }
        if (file_exists($root . "/plugins/model/{$schema}/{$this->name}.php")) {
            die($this->Prints('Model already exists! Please update instead creating new one.', true, 'light_red'));
        }
