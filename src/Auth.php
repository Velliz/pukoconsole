...
class Auth
{
    ...
    {
        if ($value === null) {
            die($this->Prints("class_name not specified! example: php puko setup auth UsersAuth", true, 'light_red'));
        }
    ...
    {
        return $this->Prints("Auth template with name {$this->value} created!", true, 'green');
    }
}
